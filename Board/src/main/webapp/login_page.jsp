...
// 	int count = dao2.getPostCount(); //한 페이지에 들어갈 데이터(테이블의 행)의 개수
	...
	%>
	<a href="login_page.jsp?page=1">처음</a>
	<%
	} else { //첫페이지면 안눌리게
	%>
	...
	<a href="login_page.jsp?page=<%=((pb - 1) * Page.PAGE_BLOCK)%>">이전</a>
	<%
	} else if (cPage2 > 1) { //그냥 이전 페이지(2페이지까지만 1페이지는 이전 페이지가 없기 때문에)
	%>

	<a href="login_page.jsp?page=<%=(cPage2 - 1)%>">이전</a>
	<%
	}
	} else {
	%>
	...
	<a style="font-weight: bolder;"
		href="login_page.jsp?page=<%=i%>"><%=i%></a>
	<%
	} else {
	%>
	...
	%>
	<a href="login_page.jsp?page=<%=(pb * Page.PAGE_BLOCK + 1)%>">다음</a>
	<%
	} else if (cPage2 < tp1) {//if조건이 아니고 && 마지막 페이지보다 작으면 다음 페이지로 넘어감
	%>
	<a href="login_page.jsp?page=<%=cPage2 + 1%>">다음</a>
	<%
	} else {
	%>
	...
	if (cPage2 < tp1) {
	%>
	<a href="login_page.jsp?page=<%=tp1%>">마지막</a>
	<%
	} else {
	%>
