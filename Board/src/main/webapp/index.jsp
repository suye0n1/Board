...
<body>
	...
	if(searchWord == null){	//검색어가 null일 경우
		%><a href = "index.jsp?page=<%=prevPage%>">[이전]</a>
	<% 
		
	}else{	//검색어 기능 
	%>
	<a href = "index.jsp?page=<%=prevPage%>&word=<%=searchWord%>">[이전]</a>
<% 
}
	}
	//블럭 적용
...
%>				
		<a href="index.jsp?page=<%=i %>&word=<%=urlEncodedSearchWord%>">[<%=i %>]</a>		
<% 
	}
		}
	}
	...
	if(hasNext){
		if(searchWord == null){
%>			<a href = "index.jsp?page=<%=nextPage%>">[다음]</a>
<%
		}else{
		%>
		<a href ="index.jsp?page=<%=nextPage%>&word=<%=searchWord%>">[다음]</a>
<% 
	}
}
